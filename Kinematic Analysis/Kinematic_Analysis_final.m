...
theta1 = degtorad(-25):0.009:degtorad(0);
theta2 = degtorad(-20):0.009:degtorad(0);
theta3 = (2/3)*theta2; % DIP = 2/3 PIP

save('theta1_init.mat','theta1');
save('theta2_init.mat','theta2');
save('theta3_init.mat','theta3');
...
% Toa do fingertip - D
P_D = P_C + [L23 0]*T(theta1(m) + theta2(n) + theta3(n));
P_Ddat = [P_Ddat;P_D];
...
HBC = 0.6979;
P_H = P_B + [L16 0]*T(HBC + theta2(n) + theta1(m));
P_Hdat = [P_Hdat;P_H];

% Toa do diem E - Goc ECD = 0.7973 rad
ECD = 0.7876;
P_E = P_C + [L26 0]*T(ECD + theta3(n) + theta2(n) + theta1(m));
P_Edat = [P_Edat;P_E];
% P_ED = P_D - P_E;
...
delta = ECD + theta1(m) + theta2(n) + theta3(n); 
deltadat(countthe) = delta;

theta1dat(countthe) = theta1(m);
theta2dat(countthe) = theta2(n);
theta3dat(countthe) = theta3(n);
